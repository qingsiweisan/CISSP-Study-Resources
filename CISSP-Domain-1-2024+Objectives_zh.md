# [领域-1](#领域-1-安全与风险管理) **安全与风险管理**

- 这个领域是最大的领域之一，权重为16%；它也是其他大多数领域的基础，因此在准备时要给予相应的时间。

- **格拉斯-斯蒂格尔法案**：于1933年通过，旨在应对股市投资的参与，将投资银行和商业银行业务分开。
  - 1999年的《格雷姆-里奇-比利雷法案》取消了《格拉斯-斯蒂格尔法案》对商业银行和投资银行之间关联的限制。
  - **安全控制评估 (SCA)**：对信息系统内的管理、操作和安全控制等不同类型的控制进行评估的过程，目的是验证控制的要求、正确实施、按预期进行操作以及结果是否符合期望。
    - 基本上是对一组定义的控制措施与基线或可靠性期望进行的正式评估；可以与安全测试和评估（ST&E）一起进行；NIST特别出版物800-53A《联邦信息系统和组织的安全和隐私控制》确保安全要求和适当安全控制的执行。
    - SCA的目标：
      - 确保安全机制的有效性
      - 评估风险管理流程的质量和彻底性
...
## [1.1](#11-理解遵守和推广专业道德-osg-10-chpts-119) 理解、遵守和推广专业道德 (OSG-10 Chpts 1,19)

- 作为一名CISSP，您必须理解并遵守(ISC)²的道德准则，以及您所在组织的道德准则。
- 1.1.1 (ISC)²专业道德准则
...
## [1.2](#12-理解和应用安全概念-osg-10-chpt-1) 理解和应用安全概念 (OSG-10 Chpt 1)

- 1.2.1 机密性、完整性、可用性、真实性和不可否认性（信息安全的五大支柱）
  - **机密性**:
    - 对象不向未经授权的主体披露的原则。
    - 用于确保数据、对象和资源机密性保护的措施概念。
    - 机密性保护在保护授权访问的同时防止信息泄露。
    - 维护对信息访问和披露的授权限制，包括保护个人隐私和专有信息的方法。
    - 敏感数据，包括个人身份信息（PII），必须保密；机密性不同于秘密。
  ...
  - **可用性**:
    ...
    - 为确保服务和数据的高可用性，可使用故障转移集群、站点弹性、自动故障转移、负载均衡、硬件和软件组件冗余以及容错等技术。
  - **真实性**: 确保传输、消息或发件人是合法的。
  ...
  - **AAA服务**:
    - 身份识别：用户身份的断言；在尝试访问安全区域或系统时声称自己是一个身份。
    - 身份验证：通过一个或多个知识、所有权或特征因素（您知道的、您拥有的、您是的）证明您是所声称的身份。
    - 授权：为特定身份或主体定义所需资源、权限（即允许/授予和/或拒绝）以及对象访问权限。
    - 问责制（又称会计，又称访问控制原则）：记录和监控的适当身份识别、身份验证和授权；访问控制过程，记录所有实体访问资源的尝试信息；审查日志文件以检查合规性和违规行为，以便让主体对其行为负责，特别是违反组织安全策略的行为。
  - 审计：记录与系统和主体相关的事件和活动的日志。

## [1.3](#13-评估应用和维持安全治理原则-osg-10-chpt-1) 评估、应用和维持安全治理原则 (OSG-10 Chpt 1)

- **安全治理**：用于在组织中做出安全决策的策略、角色、流程/实践的集合；与支持、评估、定义和指导组织的安全工作有关；它涉及确保安全策略与业务目标保持一致，并且在整个组织中是全面和一致的。
  - 安全治理是安全解决方案的实施和紧密相连的管理方法。
- **安全职能**：运营业务的一个方面，专注于评估和随着时间的推移改进安全的任务。
  - 为了管理安全，组织必须实施适当且充分的安全治理。
  - 执行风险评估以推动安全策略的行为是管理安全职能最清晰、最直接的例子。
- **第三方治理**：可能由法律、法规、行业标准、合同义务或许可要求强制执行的外部实体监督；通常涉及外部调查员或审计员。

- 1.3.1 安全职能与业务战略、目标、使命和宗旨的对齐
  - **安全管理规划**：确保正确创建/实施/执行安全策略，并与组织战略、目标、使命和宗旨保持一致；安全管理基于三种类型的计划：战略、战术和运营。
  - **战略计划**：战略计划是长期计划（有效期为5年）；它定义了组织的安全目的。
    - 战略计划应包括风险评估。
  - **战术计划**：中期计划（1年或更短），旨在为实现战略计划中设定的目标提供更多细节。
  - **运营计划**：基于战略或战术计划的短期、高度详细的计划。
    - 战略、目的、任务和目标——在层级结构中相互支持。
    - **目标**：最接近基层，代表为帮助您实现任务而付出的微小努力。
    - **任务**：代表一系列目标的集合，一个或多个任务导致目的；当您达到目的时，您就在实现战略。
  - 安全框架必须与任务和目标紧密相连，使企业能够完成其目标并推进任务，同时根据风险承受能力保护环境。

- 1.3.2 组织流程（例如，收购、剥离、治理委员会）
  - 安全治理应涉及组织的各个方面，包括收购、剥离和治理的组织流程。
  - 注意收购中的风险（因为要整合的IT环境的状态是未知的，尽职调查是关键）和剥离中的风险（如何拆分IT基础设施以及如何处理身份和凭证）。
...
- 1.3.3 组织角色和职责
  - 主要的安全角色是高级经理、安全专业人员、资产所有者、托管人、用户和审计员。
...
- 1.3.4 安全控制框架（例如，国际标准化组织（ISO）、美国国家标准与技术研究院（NIST）、信息及相关技术控制目标（COBIT）、舍伍德应用业务安全架构（SABSA）、支付卡行业（PCI）、联邦风险和授权管理计划（FedRAMP））
  - **安全控制框架**：（又称安全框架）概述了组织的安全方法，包括指南、标准和控制；安全框架对于规划组织安全解决方案的结构非常重要；框架包括：
    - **[国际标准化组织 (ISO)](https://www.iso.org/about)**：一个由160多个国家的标准机构组成的非政府组织，汇集全球专家，就各行各业的最佳实践达成一致，其六大主要产品包括：国际标准、技术报告、技术规范、公开可用规范、技术勘误和指南；ISO标准被广泛使用。
      - **[ISO/IEC 27001](https://www.iso.org/standard/27001)**：一个广受认可的国际信息安全管理体系（ISMS）标准；它提供了一种基于风险的方法，并强调ISMS的持续改进。
    ...
    - **[NIST网络安全框架 (CSF)](https://www.nist.gov/cyberframework)**：围绕六个核心功能构建：治理、识别、保护、检测、响应和恢复；为行业、政府机构和其他组织提供指导，以提高其预防、检测和响应网络攻击以及管理网络安全风险的能力。
      ...
      - **[SP 800-100](https://csrc.nist.gov/pubs/sp/800/100/r1/iprd)**：标题为《信息安全手册：管理者指南》，NIST自2006年以来未发布更新，但他们似乎正在进行更新。
    - **[COBIT (信息及相关技术控制目标)](https://www.isaca.org/resources/cobit#1)**：COBIT是ISACA创建的一个框架，专注于企业IT，使IT和业务战略保持一致，并为管理风险提供了一个全面的框架。
      ...
      - 六个关键原则：
        ...
        - 针对企业需求量身定制
        - 端到端治理体系
    - **[舍伍德应用业务安全架构 (SABSA)](https://sabsa.org/sabsa-executive-summary/)**：一个用于开发业务驱动、风险和机会导向的安全和风险管理架构的框架和方法论。
      - SABSA由一系列集成框架、模型、方法和流程组成，可独立使用或作为整体集成企业解决方案使用，包括：
        ...
        - 安全域框架
        - 全生命周期安全服务管理与绩效管理框架
    - **[支付卡行业数据安全标准 PCI DSS](https://www.pcisecuritystandards.org/standards/pci-dss/)**：一套旨在保护信用卡和借记卡信息的标准和要求。
      - 关键要素包括：
        - 建立和维护网络安全：包括防火墙、静态和传输中加密以及安全网络配置。
    ...
    - **[联邦风险和授权管理计划 (FedRAMP)](https://www.fedramp.gov/)**：一个政府范围的计划，旨在标准化云服务和产品的安全评估、授权和监控；该计划于2011年建立，旨在帮助联邦政府在保护联邦信息的同时使用云技术。
      - 关键要素包括：
        - 标准化安全框架：FedRAMP提供了一套通用的安全控制、要求和基线，云服务提供商（CSP）必须满足这些要求才能为联邦机构提供解决方案。
        - 安全评估和授权：要求CSP接受授权评估组织（3PAO）的第三方评估，以证明合规性；此过程导致操作授权（ATO）或拒绝。
        - 持续监控：FedRAMP强调持续监控的重要性，以确保安全控制随着时间的推移保持有效。
    ...
    - **[CIS关键安全控制](https://www.cisecurity.org/controls)**：CIS（互联网安全中心）关键安全控制提供了一套优先的防御威胁的行动；它专注于减少攻击面的实际步骤，例如实施安全配置、管理管理员权限和监控日志。
    - **[信息技术基础设施库 (ITIL)](https://www.axelos.com/certifications/itil-service-management/)**：ITIL是一套IT服务管理（ITSM）实践，专注于使IT服务与业务需求保持一致；它包括安全治理的要素，特别是在管理安全事件、变更和服务连续性方面，并且通常与ISO 27001等其他框架集成。
    - **特雷德韦委员会赞助组织委员会 (COSO)**：帮助组织通过建立、评估和加强其内部控制来减少财务欺诈的框架。

- 1.3.5 尽职尽责/尽职调查
  - **尽职调查**：制定计划、政策和流程以保护组织利益；尽职调查是了解应该做什么并为此进行规划；了解您的安全治理原则（政策和程序）以及组织面临的风险；供应商为证明或提供尽职尽责而采取的行动。
    - 尽职调查通常涉及：
      - 通过发现、风险评估和审查现有文档收集信息
      - 开发包含安全策略、标准、基线、指南和程序的正式安全结构
      - 建立书面政策的文档
      - 向组织传播信息
  - **尽职尽责**：实践维持尽职调查工作的各项活动；尽职尽责是指您在法律或组织政策范围内的法律责任，以实施组织的控制措施、遵守安全策略、做正确的事并做出合理的选择。
  - 安全文档是安全策略。
  - 建立治理框架后，应实施安全意识培训，包括所有新员工，他们在入职时完成安全意识培训，现有员工应定期（通常每年）重新认证。
  - 尽职尽责是对资产的负责任保护。
  - 尽职调查是证明尽职尽责的能力。
...
- 1.4.1 网络犯罪和数据泄露
  ...
  - 特别是，要熟悉：
    - **欧洲委员会网络犯罪公约**：由许多国家签署的一项条约，为网络犯罪政策制定了标准。
    - 关于数据泄露的法律，包括通知要求。
...
- 1.4.2 许可和知识产权要求
  - **知识产权**：无形资产（例如，软件、数据）。
  - **商标**：用于识别公司及其产品或服务的文字、口号和徽标。
  - **专利**：为新发明的创造者提供保护；为生产特定物品（如玩具）提供临时垄断，该物品必须新颖且独特才能获得专利。
    - **实用**：保护发明人的知识产权。
    - **外观设计**：涵盖发明的外观，有效期为15年；请注意，外观设计专利不保护发明的思想，只保护其形式，通常被认为较弱。
    - 软件：一个持续存在争议的领域；谷歌诉甲骨文案；导致了"专利流氓"的兴起。
  - **版权**：保护原创作品的作者权，例如书籍、文章、诗歌和歌曲；对艺术、音乐或文学作品的专有使用权，可防止未经授权的复制、分发或修改。
  - **许可**：软件生产商和消费者之间的合同，限制软件的使用和/或分发。
  - **商业秘密**：商业秘密法保护公司的经营秘密；商业秘密是对企业至关重要的知识产权，如果泄露给竞争对手或公众，将造成重大损害；《经济间谍法》对被判从美国公司窃取商业秘密的人处以罚款和监禁。
...
- 1.4.5 与隐私相关的问题（例如，通用数据保护条例（GDPR）、加州消费者隐私法、个人信息保护法、个人信息保护法）
  ...
  - **澄清数据海外合法使用法 (CLOUD)**：允许执法部门从美国公司收集数字证据，无论数据存储在何处。
    ...
    - 公司有机制可以质疑数据请求。
  - **电子通信隐私法 (ECPA)**：经修订后，保护有线、口头和电子通信在进行、传输和存储在计算机上时的隐私；将侵犯个人电子隐私定为犯罪，并扩大了《联邦窃听法》的范围。
...
- 1.4.6 合同、法律、行业标准和法规要求
  - 了解刑法、民法和行政法之间的区别。
    - **刑法**：保护社会免受违反我们所信仰的基本原则的行为；违反刑法的行为由联邦和州政府起诉。
    - **民法**：为个人和组织之间的商业交易提供框架；违反民法的行为由受影响的双方提交法院并进行辩论。
    - **行政法**：政府机构用来有效开展日常业务的法律。
  - **合规性**：组织可能会发现自己受到各种法律、法规机构或合同义务施加的法规的约束。
...
- 调查将根据事件类型而有所不同；例如，对于一家金融服务公司，金融系统泄露可能会导致监管调查；系统泄露或网站泄露可能会导致刑事调查；每种类型的调查都有特殊的考虑因素：
  - **行政**：内部调查通常审查运营问题或违反组织政策的行为；通常与人力资源情景相关，行政调查可能是技术故障排除的一部分；由于这些调查用于内部目的，它们在文档或程序方面的正式性和标准通常低于其他类型；行政调查通常侧重于查找运营问题的根本原因。
  - **刑事**：当犯罪发生并且您正在与执法机构合作以定罪被指控的肇事者时，就会进行刑事调查；在这种情况下，通常会为法庭收集证据，并与辩方共享证据。
    - 您需要使用确保证据可以在法庭上使用的方法来收集和处理信息。
    - 在刑事案件中，必须证明嫌疑人有罪，排除合理怀疑；与民事案件相比，这是一个更高的标准，民事案件是显示证据的优势。
  - **民事**：在民事案件中，一个人或实体起诉另一个人或实体，例如，一家公司可以起诉另一家公司侵犯商标权。
    - 民事案件通常涉及金钱赔偿，不涉及犯罪行为。
    - 在民事案件中，需要有优势证据才能获胜，这与刑事案件不同，刑事案件中嫌疑人在被证明有罪之前是无辜的，排除合理怀疑；民事调查的证据收集标准通常低于刑事案件。
  - **行业标准**：行业标准调查旨在确定组织是否遵守特定的行业标准或一组标准，例如与PCI DSS相关的标准；由于标准不是法律，这些调查可能与合同合规性有关，并且组织可能需要参与审计或评估。
    - 因为行业标准代表了众所周知且广泛实施的最佳实践，所以许多组织即使在没有被要求遵守的情况下也试图遵守这些标准，以提高安全性，并降低运营和其他风险。
  - **监管**：监管调查由监管机构进行，例如证券交易委员会（SEC）或金融业监管局（FINRA），针对涉嫌违规的组织。
...
- 1.8.3 入职、调动和离职流程
  ...
  - **终止雇佣或离职**：离职是指将员工的身份从IAM系统中删除，一旦该人离开组织；当员工调入新角色时，也可以使用此元素。
    - 无论是友好还是突然离职，前雇员都应被护送离开 premises，并且不准返回。
...
- 1.9.1 威胁和漏洞识别
  - **风险管理**：识别可能损坏或泄露数据的因素，根据数据价值和对策成本评估这些因素，并实施具有成本效益的解决方案以减轻或降低风险的过程。
  - **威胁**：任何可能对特定资产造成不良或不必要后果的潜在事件；它们可以是故意的或偶然的；粗略地将威胁视为可能对目标造成伤害的武器。
  - **漏洞**：资产中的弱点，或保障措施或对策的弱点（或缺失）；缺陷、限制、错误、脆弱性或易受伤害性。
  - 威胁和漏洞是相关的：当存在漏洞时，威胁是可能的。
    ...
    - **威胁代理/行为者**：故意利用漏洞。
    - **威胁事件**：意外发生和故意利用漏洞。
    - **威胁向量**：（又称攻击向量）是攻击或攻击者可以用来访问目标以造成伤害的路径或手段。
    - **暴露**：由于威胁而易于遭受资产损失；可能发生伤害的可能性。
    ...
    - **保障措施评估**：如果实施了保障措施，资产的ALE；实施保障措施前的ALE - 实施保障措施后的ALE - 保障措施的年成本，或 (ALE1 - ALE2) - ACS。
    - **风险**：威胁利用漏洞对资产造成伤害的可能性或概率，以及可能造成的损害严重程度；潜在伤害越大，风险越大。

- 1.9.2 风险分析、评估和范围
  - **风险评估**：用于识别风险并设定关键性优先级，然后使用风险响应来确定每个已识别风险的最佳防御措施。
  - 风险是带有漏洞的威胁。
...
- 1.9.3 风险响应和处理（例如，网络安全保险）
  - **风险响应**：为每个已识别的风险制定计划；对于给定的风险，您可以选择可能的风险响应：
    - **风险缓解**：降低风险，或风险缓解，是实施保障措施、安全控制和对策以减少和/或消除漏洞或阻止威胁。
    - **风险分配**：分配或转移风险是将因风险造成的损失责任转移到另一个实体或组织；又称风险分配和风险转移。
    - **风险威慑**：威慑是为潜在的安全和政策违反者实施威慑措施的过程。
      - 目标是说服威胁代理不要攻击。
      - 例如，实施审计、安全摄像头和警告横幅；使用保安人员。
    - **风险规避**：确定特定风险的影响或可能性太大，无法被潜在利益所抵消，并且由于该决定而不执行特定的业务功能；选择风险低于默认、常见、权宜或廉价选项的备选方案或活动的过程。
    - **风险接受**：成本/效益分析确定对策成本将超过因风险造成的可能损失成本后的结果。
      - 也意味着管理层已同意接受风险实现时的后果/损失。
    - **风险拒绝**：拒绝风险或忽视风险是对风险的不可接受的可能响应；否认风险存在并希望它永远不会实现不是对风险的有效审慎的尽职调查/尽职尽责的响应。
    - **风险转移**：向外部方（即保险公司）支付费用以接受给定风险的财务影响。
  - **固有风险**：在执行任何风险管理工作之前，环境中、系统中或产品中存在的自然、原生或默认风险水平（又称初始或起始风险）；这是风险评估过程识别的风险。
  - **残余风险**：包括管理层选择不实施的针对特定资产的威胁（管理层选择接受而不是缓解的风险）；实施安全控制后剩余的风险。
  - **总风险**：如果未实施任何保障措施，组织将面临的风险量。
  - **概念性总风险公式**：威胁 x 漏洞 x 资产价值 = 总风险。
  - **控制差距**：通过实施保障措施减少的风险量，或总风险与残余风险之间的差额。
  - **概念性残余风险公式**：总风险 - 控制差距 = 残余风险。
  - 应定期重新评估风险以维持合理的安全性，因为安全性会随着时间的推移而变化。
  - **对策**：又称"控制"或"保障措施"，可以帮助降低风险。
...
- 1.9.4 适用的控制类型（例如，预防性、检测性、纠正性）
  - 实施缓解控制的三种方法：
    - **行政**：由组织的安全策略和其他法规或要求定义的策略和程序。
    - **技术/逻辑**：示例包括防火墙、自动备份、加密。
    - **物理**：专注于为设施和现实世界对象提供保护的安全机制。
  - 保障措施：
    - **预防性**：部署预防性或预防性控制以阻止或停止不希望或未经授权的活动发生。
    - **威慑性**：部署威慑性控制以阻止违反安全策略的行为；威慑性和预防性控制相似，但威慑性控制通常依赖于说服个人不要采取不希望的行动。
    - **指令性**：部署指令性控制以指导、限制或控制主体的行动，以强制或鼓励遵守安全策略。
  - 对策：
    - **检测性**：部署检测性控制以发现或检测不希望或未经授权的活动；检测性控制在事后操作。
    - **纠正性**：在发生不希望或未经授权的活动后，纠正性控制修改环境以使系统恢复正常；它试图纠正由安全事件引起的任何问题。
    - **恢复性**：纠正性控制的扩展，但具有更高级或更复杂的能力；恢复性控制试图在违反安全策略后修复或恢复资源、功能和能力。
    - 与纠正性控制相比，恢复性控制通常处理更重大的破坏性事件，尤其是在可能发生安全违规的情况下。
    - **补偿性**：部署补偿性控制以向其他现有控制提供各种选项，以帮助执行和支持安全策略。
...
- 1.9.7 报告（例如，内部、外部）
  - 风险报告是在风险分析结束时执行的一项关键任务（即，生成和呈现总结报告）。
  - 风险登记册或风险日志是盘点组织或系统或单个项目内所有已识别风险的文档。
...
- 1.9.8 持续改进（例如，风险成熟度建模）
  ...
  - **风险成熟度模型 (RMM)**：评估成熟、可持续和可重复的风险管理流程的关键指标和活动，通常将风险成熟度评估与五级模型（例如以下模型）相关联：
    - **临时**：所有组织启动风险管理的混乱起点。
    - **初步**：进行了松散的尝试以遵循风险管理流程，但每个部门可能以独特的方式执行风险评估。
    - **已定义**：在组织范围内采用通用或标准化的风险框架。
    - **集成**：风险管理操作已集成到业务流程中，使用指标收集有效性数据，并且风险被视为业务战略决策中的一个要素。
    - **优化**：风险管理侧重于实现目标，而不仅仅是应对外部威胁；增加了旨在业务成功而不仅仅是避免事件的战略规划；并将经验教训重新整合到风险管理流程中。
...
- 1.9.9 风险框架（例如，国际标准化组织（ISO）、美国国家标准与技术研究院（NIST）、信息及相关技术控制目标（COBIT）、舍伍德应用业务安全架构（SABSA）、支付卡行业（PCI））
  ...
  - RMF旨在作为一个风险管理流程，以识别和应对威胁，并在三个核心的、相互关联的特别出版物中定义：
    ...
    - [SP 800-30 Rev 1，风险评估指南](https://csrc.nist.gov/pubs/sp/800/30/r1/final) 概述了进行风险评估的四个主要步骤：
      ...
      - 维持评估
    - **RMF有7个步骤**和**六个循环阶段**：
      - **准备** 从组织和系统层面执行RMF，通过建立管理安全和隐私风险的背景和优先级。
      - **分类** 基于损失影响分析对系统以及系统处理、存储和传输的信息进行分类。
      - **选择** 为系统选择一组初始控制，并根据风险评估将控制调整为可接受的水平。
      - **实施** 控制，并描述控制在系统及其操作环境中的使用方式。
      - **评估** 控制以确定控制是否正确实施、按预期操作并产生满足安全和隐私要求的预期结果。
      - **授权** 基于对组织运营和资产、个人以及其他组织和国家的风险可接受的确定，授权系统或通用控制。
      - **监控** 系统和相关控制，持续进行，包括评估控制有效性、记录系统和操作环境的变化、进行风险评估和影响分析以及报告系统的安全和隐私状况。
    - 请参阅我的概述文章，[NIST风险管理框架](https://blog.balancedsec.com/p/the-nist-risk-management-framework)
  - 还有其他风险框架，例如ISO/IEC 31000、ISO/IEC 31004、COSO的ERM、ISACA的Risk IT、OCTAVE、FAIR和TARA（转移、规避、减少和接受）；熟悉框架及其目标。

## [1.10](#110-理解和应用威胁建模概念和方法论-osg-10-chpt-1) 理解和应用威胁建模概念和方法论 (OSG-10 Chpt 1)

- **威胁建模**：识别、分类和分析潜在威胁的安全流程；可以在设计和开发期间作为**主动**措施（又称**防御性方法**）执行，也可以在产品部署后作为**被动**措施（又称**对抗性方法**）执行。
  - 威胁建模识别潜在危害、发生概率、关注优先级以及消除或减少威胁的方法。
- [**MITRE ATT&CK (对抗性战术、技术和常识)**](https://attack.mitre.org/)：一个全面的框架，拥有一个全球可访问的知识库，记录了网络对手使用的真实世界的战术、技术和程序（TTP）；它被组织广泛用于改进威胁检测、事件响应和网络安全防御；ATT&CK在许多软件包中用作默认模型。
- 微软使用**安全开发生命周期**（SDL），其座右铭是："设计安全，默认安全，部署和通信安全"。
  - 它有两个目标：
    - 减少与安全相关的设计和编码缺陷的数量
    - 减少任何剩余缺陷的严重性
...
- 微软开发了**STRIDE威胁模型**：
  - 欺骗：一种旨在通过使用伪造身份来访问目标系统的攻击。
  - 篡改：任何导致未经授权更改或操纵数据的行为，无论是在传输中还是在存储中。
  - 否认：用户或攻击者通过保持可信的否认来否认执行了某项操作或活动的能力。
  - 信息泄露：向外部或未经授权的实体泄露或分发私人、机密或受控信息。
  - **拒绝服务 (DoS)**：一种试图阻止对资源的授权使用的攻击；这可以通过漏洞利用、连接过载或流量泛滥来实现；例如，SYN泛滥是一种破坏TCP三次握手的DoS攻击。
  - 权限提升：一种将有限用户帐户转换为具有更高权限、权力和访问权限的帐户的攻击。
  - STRIDE是威胁分类模型；威胁分类是应用程序威胁建模的重要组成部分。
- **攻击模拟和威胁分析流程 (PASTA)**：一种七阶段的威胁建模方法：
  - 第一阶段：风险分析目标定义 (DO)
  - 第二阶段：技术范围定义 (DTS)
  - 第三阶段：应用程序分解和分析 (ADA)
  - 第四阶段：威胁分析 (TA)
...
- **可视化、敏捷和简单的威胁 (VAST)**：一种威胁建模概念，可将威胁和风险管理集成到可扩展的敏捷编程环境中。
- 安全团队的部分工作是使用不同的方法识别威胁：
  - 关注攻击者：这在特定情况下是一种有用的方法。
    - 例如，假设一名开发人员的雇佣被终止，并且在离职后和审查开发人员的计算机后，确定该人心怀不满和愤怒。
    - 将这种情况理解为可能的威胁，可以采取缓解措施。
  - 关注资产：组织最有价值的资产很可能成为攻击者的目标。
  - 关注软件：组织在内部开发应用程序，可以被视为威胁环境的一部分；目标不是识别所有可能的攻击，而是关注大局，识别风险和攻击向量。
- 了解对组织的威胁可以记录潜在的攻击向量；可以使用图表来列出受威胁的各种技术。
- **缩减分析**：目的是更好地了解产品的逻辑以及与外部元素的交互，包括将系统分解为五个核心要素：信任边界、数据流路径、输入点、特权操作和安全控制细节；又称分解应用程序、系统或环境。
